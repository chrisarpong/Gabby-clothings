...
export default function InventoryTab() {
  ...
  return (
    <div className="flex flex-col h-full bg-surface-container relative">
      <div className="p-8 border-b border-outline-variant/30 flex justify-between items-center bg-brand-bone/50 sticky top-0 z-10">
        <div>
          <h1 className="font-serif text-3xl text-primary tracking-wide">Catalog & Inventory</h1>
          <p className="font-sans text-on-surface/70 text-sm mt-2">Manage products, variants, SEO, and grouping catalogs.</p>
        </div>
        <div className="flex gap-4">
          <button 
            onClick={() => { resetCatalogForm(); setIsCatalogFormOpen(true); }}
            className="flex items-center gap-2 bg-brand-bone text-primary px-6 py-3 font-sans text-xs tracking-widest uppercase hover:bg-brand-bone/80 transition-colors border border-primary rounded-none"
          >
            <Plus className="w-4 h-4" /> Add Catalog
          </button>
          <button 
            onClick={() => { resetProductForm(); setIsProductFormOpen(true); }}
            className="flex items-center gap-2 bg-primary text-brand-bone px-6 py-3 font-sans text-xs tracking-widest uppercase hover:bg-primary transition-colors border border-primary rounded-none"
          >
      ...
      <div className="border-b flex">
         <button 
           className={`flex-1 p-4 font-serif text-lg ${activeSubTab === 'products' ? 'border-b-2 border-primary text-primary' : 'text-outline'}`}
           onClick={() => setActiveSubTab('products')}
         >Products Manager</button>
         <button 
           className={`flex-1 p-4 font-serif text-lg ${activeSubTab === 'catalogs' ? 'border-b-2 border-primary text-primary' : 'text-outline'}`}
      ...
      <div className="p-8 overflow-y-auto flex-1">
        ...
        {activeSubTab === 'products' ? (
          products === undefined ? (
            <div className="flex justify-center items-center py-20 text-on-surface-variant">Loading products...</div>
          ) : products.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              <PackageOpen className="w-16 h-16 text-primary/20 mb-6 stroke-[1]" />
              <h3 className="font-serif text-2xl text-primary mb-2">No Products Found</h3>
            </div>
          ) : (
            <div className="flex flex-col gap-4">
              {/* Toolbar */}
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-brand-bone/20 p-4 border border-outline-variant/30">
                <div className="flex items-center gap-4 w-full md:w-auto">
                  <div className="relative flex-1 md:w-64">
                    ...
                    <input 
                      ...
                      className="w-full pl-10 pr-4 py-2 border text-sm focus:outline-none focus:border-primary"
                      value={searchQuery}
                  ...
                  <div className="relative">
                    ...
                    <select 
                      className="pl-10 pr-4 py-2 border text-sm appearance-none bg-surface-container focus:outline-none focus:border-primary"
                ...
                {selectedProductIds.length > 0 && catalogs && catalogs.length > 0 && (
                  <div className="flex items-center gap-2 bg-brand-gold/10 p-2 border border-brand-gold/20">
                    ...
                    <select 
                      className="text-sm py-1 px-2 border border-outline-variant/30 bg-surface-container"
                      value={bulkCatalogId}
                    ...
                    <button 
                      ...
                      className="bg-primary text-white text-xs px-4 py-1.5 uppercase tracking-widest hover:bg-primary disabled:opacity-50 transition-colors"
                    >
                      Assign
                    </button>
                  </div>
              ...
              {(() => {
                ...
                if (filteredProducts.length === 0) {
                  return <div className="text-center py-10 text-on-surface-variant border border-outline-variant/30 bg-surface-container">No products match your search or filter.</div>;
                }
                ...
                return Object.entries(groups).map(([catName, catProducts]) => (
                  <div key={catName} className="mb-8 border border-outline-variant/30 bg-surface-container">
                    <div className="bg-brand-bone/50 px-6 py-3 border-b border-outline-variant/30">
                      <h3 className="font-serif text-xl text-primary capitalize">{catName} <span className="text-sm text-on-surface-variant font-sans tracking-widest uppercase ml-2">({catProducts.length})</span></h3>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-left font-sans text-sm border-collapse">
                        <thead>
                          <tr className="border-b border-outline-variant/30 text-on-surface uppercase tracking-widest text-[10px]">
                            ...
                            >
                              ...
                              <td className="px-6 py-4 flex items-center gap-3">
                                ...
                                {product.images && product.images.length > 0 ? (
                                  <div className="w-10 h-12 bg-surface-variant/30 flex-shrink-0">
                                    <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
                                  </div>
                                ) : (
                                  <div className="w-10 h-12 bg-brand-bone flex items-center justify-center flex-shrink-0">
                                    <ImageIcon className="w-4 h-4 text-on-surface/30" />
                                  </div>
                                )}
                                <div>
                                  <p className="text-primary font-medium flex items-center gap-2">
                                    {product.name}
                                    <Edit2 className="w-3 h-3 text-on-surface/30 opacity-0 group-hover:opacity-100 transition-opacity" />
                                  </p>
                                  <p className="text-[10px] uppercase tracking-widest text-on-surface-variant mt-1">{product.type === 'showcase_template' ? 'Custom Tailored (Made-to-Measure)' : 'Off-the-rack (Ready Size)'}</p>
                                </div>
                              </td>
                              ...
                              <td className="px-6 py-4">
                                <span className={`inline-block px-2 py-1 text-[10px] uppercase tracking-wider rounded-none ${
                                  ...
                                  product.status === 'draft' ? 'bg-surface-variant/30 text-on-surface' :
                              ...
                              <td className="px-6 py-4 text-right text-on-surface">GH₵{(product?.basePrice ?? 0).toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          /* Catalogs View */
          catalogs === undefined ? (
            <div className="flex justify-center items-center py-20 text-on-surface-variant">Loading catalogs...</div>
          ) : viewingCatalogId ? (
            <div className="flex flex-col gap-6 bg-surface-container border border-outline-variant/30 p-8">
              <div className="flex justify-between items-start border-b border-outline-variant/30 pb-6">
                <div>
                  <button onClick={() => setViewingCatalogId(null)} className="text-on-surface-variant hover:text-primary text-sm mb-4 flex items-center gap-2 uppercase tracking-widest">
                    ← Back to Catalogs
                  </button>
                  <h2 className="font-serif text-3xl text-primary mb-2">{catalogs.find(c => c._id === viewingCatalogId)?.name}</h2>
                  <p className="text-on-surface/70 text-sm max-w-2xl">{catalogs.find(c => c._id === viewingCatalogId)?.description || 'No description provided.'}</p>
                </div>
                <button 
                  onClick={() => editCatalog(catalogs.find(c => c._id === viewingCatalogId)!)}
                  className="bg-brand-bone text-primary px-6 py-3 font-sans text-xs tracking-widest uppercase hover:bg-brand-bone/80 transition-colors border border-primary rounded-none"
                >
              ...
              <div>
                <h3 className="font-serif text-xl text-primary mb-4">Products in this Catalog</h3>
                {(() => {
                  const catalogProducts = products?.filter((p: any) => p.catalogIds?.includes(viewingCatalogId)) || [];
                  if (catalogProducts.length === 0) {
                    return <div className="text-center py-12 text-on-surface-variant border border-outline-variant/30 bg-brand-bone/10">No products have been assigned to this catalog yet.</div>;
                  }
                  ...
                  return (
                    <div className="overflow-x-auto border border-outline-variant/30">
                      <table className="w-full text-left font-sans text-sm border-collapse">
                        <thead>
                          <tr className="bg-brand-bone/50 border-b border-outline-variant/30 text-on-surface uppercase tracking-widest text-[10px]">
                        ...
                        <tbody className="divide-y divide-brand-espresso/5">
                          {catalogProducts.map((product: any) => (
                            <tr key={product._id} className="hover:bg-brand-bone/20 transition-colors">
                              ...
                              <td className="px-6 py-4">
                                <span className={`inline-block px-2 py-1 text-[10px] uppercase tracking-wider rounded-none ${
                                  product.status === 'active' ? 'bg-green-500/20 text-green-500' : 'bg-surface-variant/30 text-on-surface'
                                }`}>
                                  {product.status}
                                </span>
                              </td>
                              <td className="px-6 py-4 text-on-surface">GH₵{(product.basePrice ?? 0).toFixed(2)}</td>
                              <td className="px-6 py-4 text-right">
                                <button 
                                  onClick={() => handleRemoveFromCatalog(product._id, product.catalogIds, viewingCatalogId)}
                                  className="text-red-500 hover:text-red-500 text-xs tracking-widest uppercase"
                                >
                                  Remove
                                </button>
                              </td>
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center">
              <FolderOpen className="w-16 h-16 text-primary/20 mb-6 stroke-[1]" />
              <h3 className="font-serif text-2xl text-primary mb-2">No Catalogs Found</h3>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {catalogs.map(cat => (
                <div 
                  key={cat._id} 
                  className="border p-6 relative group bg-surface-variant hover:bg-surface-container transition-colors cursor-pointer"
                  onClick={() => setViewingCatalogId(cat._id)}
                >
                   <h3 className="font-serif text-xl mb-2">{cat.name}</h3>
                   <p className="text-sm text-on-surface-variant mb-4">{cat.slug}</p>
                   <p className="text-xs text-on-surface-variant mb-4">{products?.filter((p: any) => p.catalogIds?.includes(cat._id)).length || 0} Products</p>
                   <span className={`inline-block px-2 py-1 text-[10px] uppercase tracking-wider ${cat.status === 'active' ? 'bg-green-500/20 text-green-500' : 'bg-gray-200'}`}>
                     {cat.status}
                   </span>
                   <button 
                     onClick={(e) => { e.stopPropagation(); editCatalog(cat); }} 
                     className="absolute top-4 right-4 text-outline hover:text-primary opacity-0 group-hover:opacity-100 bg-surface-container p-2 border"
                   >
      ...
      {/* Product Form Drawer */}
      {isProductFormOpen && (
        <div className="absolute inset-0 bg-primary/20 backdrop-blur-sm z-50 flex justify-end">
          <div className="w-[600px] h-full bg-surface-container shadow-2xl border-l flex flex-col pt-8">
            ...
            <form onSubmit={handleProductSubmit} className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
                ...
                <div className="flex bg-brand-bone p-1">
                  <button 
                    ...
                    className={`flex-1 py-2 text-xs uppercase tracking-widest transition-colors ${productForm.type === 'showcase_template' ? 'bg-primary text-white' : 'text-on-surface hover:bg-primary/5'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex-1 py-2 text-xs uppercase tracking-widest transition-colors ${productForm.type === 'ready_to_wear' ? 'bg-primary text-white' : 'text-on-surface hover:bg-primary/5'}`}
                  >
               ...
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Product Name</label>
               ...
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Description</label>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">
                  ...
                  {productForm.type === 'ready_to_wear' ? (
                    <div>
                      <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Stock Count</label>
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Fabric Required (Yards)</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Status</label>
                  ...
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Category</label>
                    {!isCustomCategory ? (
                      <select 
                        ...
                        className="w-full border p-3 capitalize bg-surface-container"
                      >
                      ...
                      <div className="flex gap-2">
                        ...
                        <button 
                          ...
                          className="border px-4 bg-brand-bone text-on-surface text-xs hover:bg-brand-bone/80 transition-colors uppercase tracking-widest"
                        >
                          Cancel
                        </button>
                      </div>
               ...
               {/* Gallery Uploader */}
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Image Gallery</label>
                  <div className="border-2 border-dashed border-outline-variant/30 p-8 text-center hover:bg-brand-bone/30 transition-colors cursor-pointer relative">
                    <input 
                      type="file" 
                      multiple 
                      accept="image/*"
                    ...
                    />
                    <ImageIcon className="w-8 h-8 text-on-surface/30 mx-auto mb-2" />
                    <p className="text-sm text-on-surface/70">Click or drag images to upload</p>
                  </div>
                      ...
                      {imagePreviews.map((src, idx) => (
                        <div key={idx} className="w-20 h-24 flex-shrink-0 relative group">
                          ...
                          <button 
                            ...
                            className="absolute -top-2 -right-2 bg-red-950/200 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        </div>
               ...
               {/* Catalog Assignment */}
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Assign to Catalogs</label>
               ...
               {/* SEO */}
               <div className="p-4 bg-surface-variant border">
               ...
               <div className="pt-8 border-t mt-auto">
                 <button type="submit" disabled={isSubmitting} className="w-full bg-primary text-white py-4 uppercase tracking-widest text-xs">
      ...
      {isCatalogFormOpen && (
        <div className="absolute inset-0 bg-primary/20 backdrop-blur-sm z-50 flex justify-end">
          <div className="w-[500px] h-full bg-surface-container shadow-2xl border-l flex flex-col pt-8">
            <div className="flex justify-between items-center px-8 pb-6 border-b">
            ...
            <form onSubmit={handleCatalogSubmit} className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Catalog Name</label>
               ...
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">URL Slug</label>
                  <input type="text" value={catalogForm.slug} onChange={(e) => setCatalogForm({...catalogForm, slug: e.target.value})} className="w-full border p-3 bg-surface-variant" placeholder="e.g. summer-collection" required />
               </div>
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Description</label>
               ...
               <div>
                  <label className="block text-xs uppercase tracking-widest text-on-surface mb-2">Cover Image</label>
                  <input type="file" onChange={(e) => setCatalogImageFile(e.target.files?.[0] || null)} className="w-full border p-3" />
                  {catalogForm.coverImageId && <p className="text-xs text-on-surface-variant mt-2">Current Image ID: {catalogForm.coverImageId}</p>}
               </div>
               <div className="pt-8 border-t mt-auto">
                 <button type="submit" disabled={isSubmitting} className="w-full bg-primary text-white py-4 uppercase tracking-widest text-xs">
